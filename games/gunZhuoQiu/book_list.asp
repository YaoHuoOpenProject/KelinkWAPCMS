...
sub showsearch()
  ...
  sql=" and gametype=3 and siteid="&siteid 
...
set rs=conn.execute(sql)
	...
	if rs.eof and rs.bof then
		...
		If absPageNum <> TotalPages Then pageinfo=pageinfo&"<a href="""&http_start&"games/gunzhuoqiu/book_list.asp?action=search&amp;siteid="&siteid&"&amp;classid="&request("classid")&"&amp;page="&absPageNum+1&jid2&""">下一页</a> "
		If absPageNum > 1 Then pageinfo=pageinfo&"<a href="""&http_start&"games/gunzhuoqiu/book_list.asp?action=search&amp;siteid="&siteid&"&amp;classid="&request("classid")&"&amp;page="&absPageNum-1&jid2&""">上一页</a>"
			pageinfo=pageinfo&"<br/>"
		if TotalPages > 1 then pageinfo=pageinfo&"<input title=""页码"" name=""page"" format=""*N"" size=""3"" maxlength=""8""/><anchor><go href="""&http_start&"games/gunzhuoqiu/book_list.asp?action=search&amp;siteid="&siteid&"&amp;classid="&request("classid")&jid2&""" method=""post"" accept-charset=""utf-8""><postfield name=""page"" value=""$(page)""/></go>转到该页</anchor><br/>"
		for i=1 to pagesz
...
<a href="<%=http_start%>games/gunzhuoqiu/show.asp?siteid=<%=siteid%>&amp;classid=<%=request("classid")%><%=jid2%>">返回上级</a><br/>
