<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ClassConfigAll.aspx.cs" Inherits="KeLin.WebSite.Games.lucky28.ClassConfigAll" %><%@ Import namespace="KeLin.ClassManager.Tool" %><%
Response.Write(WapTool.showTop(this.GetLang("幸运28配置|幸运28配置|site setup"), wmlVo));//显示头                                                                                                                                                                       
if (ver == "1")
{
    Response.Write("<p>");
    ...
    Response.Write("[1]开奖时间每隔:<br/>");
    Response.Write("<input type=\"text\" name=\"par1" + r + "\" size=\"8\" value=\"" + this.par1 + "\"/><br/>");
    Response.Write("[2]压注最小币:<br/>");
    Response.Write("<input type=\"text\" name=\"par2" + r + "\" size=\"8\" value=\"" + this.par2 + "\"/><br/>");
    Response.Write("[3]压注最大币:<br/>");
    Response.Write("<input type=\"text\" name=\"par3" + r + "\" size=\"8\" value=\"" + this.par3 + "\"/><br/>");
    Response.Write("[4]当前奖池有:<br/>");
    Response.Write("<input type=\"text\" name=\"par4"+r+"\" size=\"8\" value=\"" + this.par4 + "\"/><br/>");
   
    Response.Write("[5]每期最多投:<br/>");
    Response.Write("<input type=\"text\" name=\"par5" + r + "\" size=\"8\" value=\"" + this.par5 + "\"/>注(0不限)<br/>");

    Response.Write("[6]显示聊天列表:");
    Response.Write("<input type=\"text\" name=\"par6" + r + "\" size=\"5\" value=\"" + this.par6 + "\"/><br/>");
    Response.Write("(0-10行)<br/>");
    Response.Write("[7]每期最多投:<br/>");
    Response.Write("<input type=\"text\" name=\"par7" + r + "\" size=\"8\" value=\"" + this.par7 + "\"/>"+siteVo.sitemoneyname+"(0不限)<br/>");

    Response.Write("[8]初始奖金池:<br/>");
    Response.Write("<input type=\"text\" name=\"par8" + r + "\" size=\"8\" value=\"" + this.par8 + "\"/><br/>");
    

    Response.Write("<anchor><go href=\"" + http_start + "games/lucky28/ClassConfigAll.aspx\" method=\"post\" accept-charset=\"utf-8\">");
    ...
    if (backtype == "wap")
    {
        Response.Write("<br/><a href=\"" + this.http_start + "games/lucky28/index.aspx?siteid=" + this.siteid + "&amp;classid=" + this.classid + "\">" + this.GetLang("返回上级|返回上级|Back to set") + "</a> ");
    }
...
else //2.0界面
{
    ...
    Response.Write("<div class=\"subtitle\">" + this.GetLang("幸运28配置|幸运28配置|site setup") + "</div>");
    if (this.INFO == "OK")
    {
    ...
    Response.Write("<form name=\"f\" action=\"" + http_start + "games/lucky28/ClassConfigAll.aspx\" method=\"post\">");
    Response.Write("[0]每次赢家获得赌注:<br/>");
    ...
    Response.Write("[5]每期最多投:<br/>");
    Response.Write("<input type=\"text\" name=\"par5\" size=\"8\" value=\"" + this.par5 + "\"/>注(0不限制)<br/>");

    Response.Write("[6]显示聊天列表:");
    Response.Write("<input type=\"text\" name=\"par6\" size=\"5\" value=\"" + this.par6 + "\"/><br/>");
    Response.Write("(0-10行)<br/>");
    Response.Write("[7]每期最多投:<br/>");
    Response.Write("<input type=\"text\" name=\"par7\" size=\"8\" value=\"" + this.par7 + "\"/>" + siteVo.sitemoneyname + "(0不限)<br/>");
