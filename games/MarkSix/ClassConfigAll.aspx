<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ClassConfigAll.aspx.cs" Inherits="KeLin.WebSite.Games.marksix.ClassConfigAll" %><%@ Import namespace="KeLin.ClassManager.Tool" %><%
Response.Write(WapTool.showTop(this.GetLang("博彩配置|博彩配置|site setup"), wmlVo));//显示头                                                                                                                                                                       
if (ver == "1")
{
    Response.Write("<p>");
    ...
    Response.Write("<br/><a href=\"" + this.http_start + "games/marksix/classconfigall.aspx?siteid=" + this.siteid + "&amp;classid=" + this.classid + "&amp;sid=" + this.sid1 + "-2-" + this.cs + "-" + this.lang + "-" + this.myua + "-"+this.width+  "\">进入WAP2.0界面配置参数</a> <br/>");
   


    if (backtype == "wap")
    {
        Response.Write("<br/><a href=\"" + this.http_start + "games/marksix/index.aspx?siteid=" + this.siteid + "&amp;classid=" + this.classid + "\">" + this.GetLang("返回上级|返回上级|Back to set") + "</a> ");
    }
...
else //2.0界面
{
    ...
    Response.Write("<div class=\"subtitle\">参数配置|<a href=\"" + this.http_start + "games/marksix/admin_bet.aspx?siteid=" + this.siteid + "&amp;classid=" + this.classid + "&amp;backtype=wap\">开奖管理</a></div>");
    ...
    Response.Write("<form name=\"f\" action=\"" + http_start + "games/marksix/ClassConfigAll.aspx\" method=\"post\">");
    if (KL_Games_Close_MarkSix == "0")
    {
        Response.Write("<b>系统设置此游戏为关闭，下面设置将无效！要开启联系超级管理员配制web.config中的参数KL_Games_Close_MarkSix设为1，建议用国内空间关闭，国外可开启。</b><br/>");
    }
    ...
    Response.Write("[22]标题内容:");
    Response.Write("<input type=\"text\" name=\"par22\"  value=\"" + this.par22 + "\"/><br/>");

    Response.Write("[23]游戏下面说明，可加入论坛UBB链接等:<br/>");
    Response.Write("<textarea name=\"par23\" style=\"width:100%\" rows=\"5\">" + this.par23 + "</textarea><br/>");
    ...
    Response.Write("<div class=\"tip\">");
     ...
     Response.Write("单双大小:1~24(小)25~48(大),49剔除,开49(退还本金)<br/>");
     Response.Write("多个号码请用,号隔开；单位数1要改成二位数01<br/>");
     Response.Write("域名访问可以直接访问博彩首页吗？可以的。WEB后台超级管理员区,全局设置，地址转发。");
     Response.Write("</div>");
