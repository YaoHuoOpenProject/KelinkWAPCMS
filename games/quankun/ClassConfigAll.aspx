<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ClassConfigAll.aspx.cs" Inherits="KeLin.WebSite.Games.quankun.ClassConfigAll" %><%@ Import namespace="KeLin.ClassManager.Tool" %><%
Response.Write(WapTool.showTop(this.GetLang("乾坤宝配置|乾坤宝配置|site setup"), wmlVo));//显示头                                                                                                                                                                       
if (ver == "1")
{
    Response.Write("<p>");
    ...
    Response.Write("[13]每期最多投:");
    Response.Write("<input type=\"text\" name=\"par13" + r + "\" size=\"5\" value=\"" + this.par13 + "\"/>次(0不限制)<br/>");
    Response.Write("[14]每期最多投:");
    Response.Write("<input type=\"text\" name=\"par14" + r + "\" size=\"5\" value=\"" + this.par14 + "\"/>" + siteVo.sitemoneyname + "(0不限制)<br/>");

    Response.Write("<anchor><go href=\"" + http_start + "games/quankun/ClassConfigAll.aspx\" method=\"post\" accept-charset=\"utf-8\">");
    ...
    if (backtype == "wap")
    {
        Response.Write("<br/><a href=\"" + this.http_start + "games/quankun/index.aspx?siteid=" + this.siteid + "&amp;classid=" + this.classid + "\">" + this.GetLang("返回上级|返回上级|Back to set") + "</a> ");
    }
...
else //2.0界面
{
    ...
    Response.Write("<div class=\"subtitle\">" + this.GetLang("乾坤宝配置|乾坤宝配置|site setup") + "</div>");
    if (this.INFO == "OK")
    {
    ...
    Response.Write("<form name=\"f\" action=\"" + http_start + "games/quankun/ClassConfigAll.aspx\" method=\"post\">");
    Response.Write("[0]每次赢家获得赌注:<br/>");
    ...
    Response.Write("[6]6点1赔:<br/>");
    Response.Write("<input type=\"text\" name=\"par6\" size=\"8\" value=\"" + this.par6 + "\"/><br/>");
    ...
    Response.Write("(0-10)<br/>");
    Response.Write("[13]每期最多投:");
    Response.Write("<input type=\"text\" name=\"par13\" size=\"5\" value=\"" + this.par13 + "\"/>次(0不限制)<br/>");
    Response.Write("[14]每期最多投:");
    Response.Write("<input type=\"text\" name=\"par14\" size=\"5\" value=\"" + this.par14 + "\"/>" + siteVo.sitemoneyname + "(0不限制)<br/>");
