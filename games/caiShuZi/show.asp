...
function getpic(p)
'p=234
p=replace(p,"0","<img src=""0.gif"" alt=""0""/>")
...
p=replace(p,"7","<img src=""7.gif"" alt=""7""/>")
p=replace(p,"8","<img src=""8.gif"" alt=""8""/>")
p=replace(p,"9","<img src=""9.gif"" alt=""9""/>") 
getpic=p
end function
%><wml>
<head>
...
<%if id<>"" then%>
<card title="等待..." ontimer="<%=http_start%>games/caishuzi/result.asp?mymoney=<%=mymoney%>&amp;beishu=<%=beishu%>&amp;id=<%=id%>&amp;classid=<%=classid%>&amp;siteid=<%=siteid%><%=jid2%>">
	<timer value="50"/>
		<p><b>等待对方出三个号码...</b>(5秒内)
<%else%>
<card id="main" title="猜数字"><p>
...
<%end if%>
<% if userid="" then %>
请先<a href="<%=http_start%>waplogin.aspx?siteid=<%=siteid%>&amp;classid=<%=classid%>">登录</a>后再玩。<br/>
<%end if%>
对方出三个数字，让你猜总和！<br/>
...
<anchor>确定啦
 <go href="<%=http_start%>games/caishuzi/show.asp" method="post" accept-charset="utf-8">
...
<%end if%>
<br/>
<br/><a href="<%=http_start%>games/caishuzi/book_list.asp?siteid=<%=siteid%>&amp;classid=<%=classid%><%=jid2%>">排行榜</a>
...
<p>
猜数字游戏是一种简单有趣的数字竞猜游戏。中奖结果为（0－9）的三个数字中随机选出三个数字之和，共有 28 种结果（0－27），投注的注数不限制。投注可大可小，赔率1:1。
