...
<%
 set rsq=Server.CreateObject("ADODB.Recordset")
  ssql="select * from wap_game_chuiniu where tzz=" & userid & " and siteid="&siteid&" order by id desc"
  rsq.open ssql,conn,3
  if rsq.eof then%>
  这年头竟然没玩过吹牛,<a href="<%=http_start%>games/chuiniu/add.asp?siteid=<%=siteid%>&amp;classid=<%=classid%><%=jid2%>">赶紧吹一个吧!</a><br/>
  <%
  else
rsq.pagesize=8
...
response.write("<a href='"&http_start&"games/chuiniu/ls.asp?siteid="&siteid&"&amp;classid="&classid&jid2&"&amp;page=" & epage+1 & "'>下页</a> ")
end if
if epage>1 then
response.write("<a href='"&http_start&"games/chuiniu/ls.asp?siteid="&siteid&"&amp;classid="&classid&jid2&"&amp;page=" & epage-1 & "'>上页</a>")
end if
%>
