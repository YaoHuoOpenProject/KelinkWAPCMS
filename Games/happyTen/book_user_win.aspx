<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="book_user_win.aspx.cs" Inherits="KeLin.WebSite.Games.happyTen.book_user_win" %><%@ Import namespace="KeLin.ClassManager.Tool" %><%
...
if (ver == "1")
{
    ...
    if (this.INFO == "OK")
    {
        ...
        strhtml.Append("</b> ");
        strhtml.Append("<a href=\"" + this.http_start + "games/happyTen/index.aspx?siteid=" + siteid + "&amp;classid=" + classid + "&amp;nid=" + id + "&amp;lpage=" + lpage + "" + "\">返回</a><br/>");
    ...
    if (this.INFO == "")
    {
        for (int i = 0; (listVo != null && i < listVo.Count); i++)
        {
            ...
            strhtml.Append(string.Format("{0:dd日HH:mm}", listVo[i].betTime) + "[" + listVo[i].years + "年第" + listVo[i].peroid + "期]" + "<a href=\"" + this.http_start + "bbs/userinfo.aspx?siteid=" + this.siteid + "&amp;touserid=" + listVo[i].userid + "&amp;backurl=" + HttpUtility.UrlEncode(this.GetUrlQueryString()) + "\">" + ShowNickName_color(listVo[i].userid, listVo[i].nickname) + "(" + listVo[i].userid + ")</a>投注[" + GamesClassManager.Tool.GetHappyTen_PlayName(listVo[i].types.ToString()) + ":" + listVo[i].types_content + "]共" + listVo[i].num + "注，每注金额" + listVo[i].betMoney + "；结果:");
            if (listVo[i].state == 0)
            {
                strhtml.Append("待开奖");
            }
    ...
    //显示导航分页
    strhtml.Append("----------<br/>");
    strhtml.Append("【共赢:" + allmoney.ToString("f2") + "个" + siteVo.sitemoneyname + " <a href=\"" + this.http_start + "games/happyTen/book_user_win.aspx?action=add&amp;siteid=" + this.siteid + "&amp;classid=" + this.classid + "\">确定兑奖</a>】");
