<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="book_list.aspx.cs" Inherits="KeLin.WebSite.video.book_list" %><%@ Import namespace="KeLin.ClassManager.Tool" %><%
...
if (ver == "1")
{
    ...
    for (int i = 0; (listVo != null && i < listVo.Count); i++)
    {
        ...
        strhtml.Append(index + "." + tempstr + "<a href=\"" + http_start + "video/book_view.aspx?siteid=" + siteid + "&amp;classid=" + listVo[i].book_classid + "&amp;id=" + listVo[i].id + "&amp;lpage=" + CurrentPage + "" + "\">" + listVo[i].book_title + book_click + "</a><br/>");
        ...
        if (listVo[i].book_img != "" && showimg== "0")
        {
            ...
            {
                strhtml.Append("<img src=\"" + this.http_start + "video/" + listVo[i].book_img + "\" alt=\"load...\"/><br/>");
            }
        }
    ...
    if (action == "search")
    {
        strhtml.Append("<a href=\"" + this.http_start + "video/book_search.aspx?siteid=" + siteid + "&amp;classid=" + classid + "" + "\">返回搜索</a> <br/>");
    }
    if (downLink != "")
    {
...
else //2.0界面
{
    ...
    if (buju != "0")
    {
        ...
        for (int i = 0; (listVo != null && i < listVo.Count); i++)
        {
            ...
            {
                vo.imageURL = this.http_start + "video/" + listVo[i].book_img;
            }
            if (WapTool.ISAPI_Rewrite3_Open == "1")
            {
                vo.linkURL = http_start + "video-" + listVo[i].id +".html"+ lpagetemp;
            
            }
            else
            {
                vo.linkURL = http_start + "video/view.aspx?id=" + listVo[i].id + lpagetemp;
            }
            vo.time = listVo[i].book_date.ToString();
            pList.Add(vo);
        }
        strhtml_list.Append(Page_Layout.GetListLayout(pList, buju + "*" + buju_width + "*" + buju_height, wmlVo, "video"));
    }
    else
    {
        //显示列表
        ...
        for (int i = 0; (listVo != null && i < listVo.Count); i++)
        {
            ...
            if (WapTool.ISAPI_Rewrite3_Open == "1")
            {
                strhtml_list.Append(tempstr + "<a href=\"" + http_start + "video-" + listVo[i].id +".html"+ lpagetemp + "\">" + listVo[i].book_title + book_click + "");
           
            }
            else
            {
                strhtml_list.Append(tempstr + "<a href=\"" + http_start + "video/view.aspx?id=" + listVo[i].id + lpagetemp + "\">" + listVo[i].book_title + book_click + "");
            }
            if (listVo[i].book_img != "" && showimg == "0")
            {
                if (listVo[i].book_img.StartsWith("/") || listVo[i].book_img.ToLower().StartsWith("http://"))
                ...
                {
                    strhtml_list.Append("<br/><img src=\"" + this.http_start + "video/" + listVo[i].book_img + "\" alt=\"load...\"/>");
                }
            }
