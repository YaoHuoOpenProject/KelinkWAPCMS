<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Book_List.aspx.cs" Inherits="KeLin.WebSite.hotel.Book_List" %><%@ Import namespace="KeLin.ClassManager.Tool" %><%
...
if (ver == "1")
{
    ...
    strhtml.Append("<a href=\"" + http_start + "hotel/book_re.aspx?lpage=" + this.CurrentPage + "&amp;id=0&amp;classid=" + classid + "&amp;siteid=" + siteid + "" + "\">我的订单</a><br/>");
    //显示列表
    string book_click = "";
    for (int i = 0; (listVo != null && i < listVo.Count); i++)
    {
        ...
        strhtml.Append("<a href=\"" + http_start + "hotel/book_view.aspx?siteid=" + siteid + "&amp;classid=" + listVo[i].classid + "&amp;id=" + listVo[i].id + "&amp;lpage=" + CurrentPage + "" + "\">" + listVo[i].name +"["+listVo[i].city+"]"+ book_click + "</a><br/>");
    }
    if (listVo == null)
    {
    ...
    if (action == "search")
    {
        strhtml.Append("<a href=\"" + this.http_start + "hotel/book_search.aspx?siteid=" + siteid + "&amp;classid=" + classid + "" + "\">返回搜索</a> ");
    }
...
else //2.0界面
{
    ...
    strhtml.Append("<!--web-->");
    strhtml.Append("<div class=\"btBox\"><div class=\"bt1\"><a href=\"" + http_start + "hotel/book_re.aspx?lpage=" + this.CurrentPage + "&amp;id=0&amp;classid=" + classid + "&amp;siteid=" + siteid + "" + "\">我的订单</a></div></div>");
    string book_click = "";
    for (int i = 0; (listVo != null && i < listVo.Count); i++)
    {
        index = index + kk;
        ...
        if (WapTool.ISAPI_Rewrite3_Open == "1")
        {
            strhtml.Append("<a href=\"" + http_start + "hotel-" + listVo[i].id +".html"+ lpagetemp + "\">" + listVo[i].name + "[" + listVo[i].city + "]" + book_click + "</a></div>");
        
        }
        else
        {
            strhtml.Append("<a href=\"" + http_start + "hotel/view.aspx?id=" + listVo[i].id + lpagetemp + "\">" + listVo[i].name + "[" + listVo[i].city + "]" + book_click + "</a></div>");
        }
    }
