<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="user_WAPadd.aspx.cs" Inherits="KeLin.WebSite.ring.user_WAPadd" %>
...
if (ver == "1")
{
    ...
    Response.Write("<a href=\"" + this.http_start + "ring/user_wapadd.aspx?action=class&amp;siteid=" + this.siteid + "&amp;classid=" + this.classid + "&amp;smalltypeid=" + this.smalltypeid + "&amp;sid=" + this.sid1 + "-2-" + this.cs + "-" + this.lang + "-" + this.myua + "-" + this.width + "\">" + this.GetLang("进入WAP2.0界面上传|进入WAP2.0界面上传|wap2.0 add upfile") + "</a><br/>");
    Response.Write("<a href=\"" + this.http_start + "ring/book_list.aspx?siteid=" + this.siteid + "&amp;classid=" + this.classid + "&amp;smalltypeid=" + this.smalltypeid + "\">" + this.GetLang("返回列表|返回列表|Back to list") + "</a><br/>");
...
else //2.0界面
{
    ...
    Response.Write("<div class=\"subtitle\">" + this.GetLang("一对多批量上传|一对多批量上传|Add operation") + "</div>");
    ...
    if (this.INFO == "OK")
    {
        ...
        //}
        Response.Write(" <a href=\"" + this.http_start + "ring/book_list.aspx?siteid=" + this.siteid + "&amp;classid=" + this.classid + "&amp;smalltypeid=" + this.smalltypeid + "\">" + this.GetLang("返回|返回|Back to list") + "</a>");
    }
    ...
    Response.Write("<form name=\"g1\" action=\"" + http_start + "ring/user_wapadd.aspx\" method=\"get\">");
    ...
    //选择栏目
    Response.Write("<form name=\"gt\" action=\"" + http_start + "ring/user_wapadd.aspx\" enctype=\"multipart/form-data\" method=\"post\">");

    Response.Write(this.GetLang("标题|標題|Title") + "*:");
    Response.Write("<input type=\"text\" name=\"book_title\" value=\"" + book_title + "\"/><br/>");

    Response.Write(this.GetLang("专辑|专辑|专辑") + " :");
    Response.Write("<input type=\"text\" name=\"book_director\" value=\"" + book_director + "\"/><br/>");


    Response.Write(this.GetLang("歌手|歌手|歌手") + " :");
    ...
    Response.Write(this.GetLang("歌曲歌词|歌曲歌词|Introduction") + "*:<br/>");
    Response.Write("<textarea name=\"book_content\" rows=\"3\" cols=\"20\">" + book_content + "</textarea><br/>");

    
    Response.Write(this.GetLang("如果有图片|如果有图片|pic") + ":<br/>");
    ...
    {
        Response.Write("---" + this.GetLang("铃声或图片|铃声或图片|Source or picture") + (i + 1) + "---<br/>");
    ...
    Response.Write("<input type=\"hidden\" name=\"sid\" value=\"" + sid + "\"/>");
    Response.Write("<input type=\"submit\"  name=\"bt\" class=\"btn\" value=\"" + this.GetLang("上传|上传|submit") + "\"/></form>");
    Response.Write("</div>");

    Response.Write("<div class=\"btBox\"><div class=\"bt1\">");
    Response.Write("<a href=\"" + this.http_start + "ring/book_list.aspx?siteid=" + this.siteid + "&amp;classid=" + this.classid + "&amp;smalltypeid=" + this.smalltypeid + "\">" + this.GetLang("返回列表|返回列表|Back to list") + "</a>");
