<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="admin_WAPmodify.aspx.cs" Inherits="KeLin.WebSite.paimai.admin_WAPmodify" %><%@ Import namespace="KeLin.ClassManager.Tool" %><%
...
if (ver == "1")
{
    ...
    if (this.INFO == "OK")
    {
        ...
        Response.Write("</b><br/>");
        Response.Write("<a href=\"" + this.http_start + "paimai/admin_userlistWAP.aspx?siteid=" + this.siteid + "&amp;classid=" + this.classid + "&amp;page=" + this.page + "\">" + this.GetLang("返回列表|返回列表|Back to list") + "</a><br/>");
    }
    else if(this.INFO!="")
    {
        Response.Write("<b>");
    ...
    Response.Write("<a href=\"" + this.http_start + "paimai/admin_wapmodifyadd.aspx?action=class&amp;siteid=" + this.siteid + "&amp;classid=" + this.classid + "&amp;id=" + this.id + "&amp;sid=" + this.sid1 + "-2-" + this.cs + "-" + this.lang + "-" + this.myua + "-" + this.width + "\">" + this.GetLang("进入WAP2.0界面续传|进入WAP2.0界面续传|wap2.0 add upfile") + "</a><br/>");
    ...
    Response.Write("<input type=\"text\" name=\"book_jiage" + r + "\" value=\"" + bookVo.book_jiage + "\"/><br/>");
    Response.Write(this.GetLang("最底起价|最底起价|最底起价") + "*:");
    Response.Write("<input type=\"text\" name=\"book_yhjiage" + r + "\" value=\"" + bookVo.book_yhjiage + "\"/><br/>");
    Response.Write("(价格只支持整数)<br/>");
    ...
    Response.Write("<anchor><go href=\""+http_start+"paimai/admin_WAPmodify.aspx\" method=\"post\" accept-charset=\"utf-8\">");
    ...
    Response.Write("<postfield name=\"nextPage\" value=\"$(nextPage" + r + ")\"/>");
    Response.Write("<postfield name=\"sid\" value=\""+sid+"\"/>");
    Response.Write("</go>" + this.GetLang("修 改|修 改|Modify") + "</anchor><br/><br/>");


    Response.Write("<br/><a href=\"" + this.http_start + "paimai/admin_userlistWAP.aspx?siteid=" + this.siteid + "&amp;classid=" + this.classid + "&amp;page=" + this.page + "\">" + this.GetLang("返回列表|返回列表|Back to list") + "</a>");
...
else //2.0界面
{
    ...
    Response.Write("<a href=\""+this.http_start+"paimai/admin_wapmodifyadd.aspx?siteid="+this.siteid+"&classid="+this.classid+"&id="+this.id+"&sid="+this.sid+"\">续传图片</a>");
    Response.Write("</div>");
    Response.Write("<div class=\"content\">");
    Response.Write("<form name=\"go\" action=\"" + this.http_start + "paimai/admin_WAPmodify.aspx\" method=\"post\">");
    ...
    Response.Write(this.GetLang("一口价|一口价|一口价") + " :");
    Response.Write("<input type=\"text\" name=\"book_jiage\" value=\"" + bookVo.book_jiage + "\"/><br/>");
    Response.Write(this.GetLang("最底起价|最底起价|最底起价") + "*:");
    Response.Write("<input type=\"text\" name=\"book_yhjiage\" value=\"" + bookVo.book_yhjiage + "\"/><br/>");
    Response.Write("(价格只支持整数)<br/>");
    ...
    Response.Write("<a href=\"" + this.http_start + "paimai/admin_userlistWAP.aspx?siteid=" + this.siteid + "&amp;classid=" + this.classid + "&amp;page=" + this.page + "\">" + this.GetLang("返回列表|返回列表|Back to list") + "</a>");
