<%@ Page Language="C#" AutoEventWireup="true" Codebehind="userlist.aspx.cs" Inherits="KeLin.WebSite.ring.userlist" %>
...
<link href="/NetCSS/style.css" rel="stylesheet" type="text/css">
    ...
    <script language="javascript" type="text/javascript">
        ...
        function DoEdit() {
            ...
            window.open('/ring/admin_WAPmodify.aspx?action=go&id=' + edtHangBiaoShis + '&siteid=<%=base.SiteId %>&classid=' + toclassid + '&page=1&sid=<%=base.SID %>');
         ...
         **/
            window.location.href = "/admin/modifyAD.aspx?systype=ring";
        }
        function DoContent() {
        ...
        function DoSmall() {
            window.location.href="../admin/systemclass.aspx?systype=ring&classid="+document.getElementById("TextBox4").value;  
        }
        function DoMyItem() {
            var delHangBiaoShis = GetOnChecked();
            if (delHangBiaoShis == null || delHangBiaoShis == "") {
...
window.location.href="upload_bath.aspx?systype=ring&classid="+ItemID;
//window.open ("upload_bath.aspx?systype=ring&classid=<%=classid%>", "addsystemwindow", "height=450, width=550, toolbar= no, menubar=no, scrollbars=no, resizable=no, location=no, status=no");
}
function daoru() { 
                                                ...
                                                <table id="tblManageRanks" class="tableBorder" cellspacing="1" cellpadding="3" border="0"
                                                    ...
                                                    <tr>
                                                        ...
                                                        <input id="Button3" name="" type="button" value=" 审 核 " onclick="DoCheck()" class="bt"/>                                                       
                                                            ...
                                                            <input id="Button12" name="" type="button" value="更多栏目属性设置" onclick="window.open('/ring/classconfigall.aspx?classid=<%=this.classid %>&siteid=<%=base.SiteId %>&page=1&sid=<%=base.SID %>');" class="bt"/>
                                                            <input id="Button5" name="" type="button" value="插放广告" onclick="DoAd()"class="bt" />
                                                            &nbsp;
                                                            <input id="Button2" name="" type="button" value="一对一批量上传" onclick="window.open('/ring/admin_WAPadd2.aspx?num=2&action=go&classid=<%=this.classid %>&siteid=<%=base.SiteId %>&page=1&sid=<%=base.SID %>');" class="bt"/>
                                                            &nbsp;
                                                            <input id="Button11" name="" type="button" value="一对多批量上传" onclick="window.open('/ring/admin_WAPadd.aspx?num=2&action=go&classid=<%=this.classid %>&siteid=<%=base.SiteId %>&page=1&sid=<%=base.SID %>');" Class="bt" />
                                                            &nbsp;
                                                            <input id="Button1" name="" type="button" value="采集接口" onclick="window.open('/ring/admin_WAPaddText.aspx?num=2&action=go&classid=<%=this.classid %>&siteid=<%=base.SiteId %>&page=1&sid=<%=base.SID %>');" class="bt"/>
                                                            &nbsp;
                                                           <!-- <input id="Button7" name="" type="button" value="新 增" onclick="DoNew()" class="bt"/>
                                                            &nbsp;-->
                                                            <input id="Button6" name="" type="button" value="修 改 + 审核送币" onclick="DoEdit()"class="bt" />
                                                            &nbsp;
                                                         ...
                                                         <asp:TemplateColumn HeaderText="标题" HeaderStyle-CssClass="column">
                                                            <ItemTemplate>
                                                                <a href="book_view.aspx?siteid=<%#DataBinder.Eval(Container.DataItem,"userid") %>&classid=<%#DataBinder.Eval(Container.DataItem,"book_classid") %>&id=<%#DataBinder.Eval(Container.DataItem,"id") %>&sid=-2-0-0-0-480"
                                                                    target="_blank" title="查看铃声及回复">
                                                                    <%#DataBinder.Eval(Container.DataItem, "book_title")%>
                                                                </a>
                                                            </ItemTemplate>
                                                        </asp:TemplateColumn>
                                                        <asp:TemplateColumn HeaderText="格式" SortExpression="book_ext" HeaderStyle-CssClass="column">
                                                            <ItemTemplate>
                                                                <%#DataBinder.Eval(Container.DataItem, "book_ext")%>
                                                            </ItemTemplate>
                                                        </asp:TemplateColumn>
                                                        <asp:TemplateColumn HeaderText="演唱" SortExpression="book_author" HeaderStyle-CssClass="column">
                                                            <ItemTemplate>
