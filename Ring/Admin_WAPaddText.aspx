<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="admin_WAPaddText.aspx.cs" Inherits="KeLin.WebSite.ring.admin_WAPaddText" %><%@ Import namespace="KeLin.ClassManager.Tool" %>
<%
Response.Write(WapTool.showTop(this.GetLang("铃声采集接口|铃声采集接口|content add"), wmlVo));//显示头                                                                                                                                                                       
if (ver == "1")
{
    Response.Write("<p>");    
    Response.Write(this.ERROR);

    Response.Write("<a href=\"" + this.http_start + "ring/admin_wapaddText.aspx?action=class&amp;siteid=" + this.siteid + "&amp;classid=" + this.classid + "&amp;smalltypeid=" + this.smalltypeid + "&amp;sid=" + this.sid1 + "-2-" + this.cs + "-" + this.lang + "-" + this.myua + "-" + this.width + "\">" + this.GetLang("进入WAP2.0界面上传|进入WAP2.0界面上传|wap2.0 add upfile") + "</a><br/>");
...
else //2.0界面
{
    ...
    Response.Write("<div class=\"subtitle\">" + this.GetLang("铃声采集接口|铃声采集接口|Add operation") + "</div>");
    ...
    //选择栏目
    Response.Write("<form name=\"gt\" action=\"" + http_start + "ring/admin_wapaddText.aspx\" enctype=\"multipart/form-data\" method=\"post\">");
    ...
    Response.Write("铃声地址：<br/><textarea name=\"book_file\" rows=\"3\" style=\"width:100%\"></textarea><br/>");
    Response.Write("多个文件用|区别<br/>");
    
    Response.Write("缩小图:<br/><input type=\"text\" name=\"book_img\" value=\"\"/><br/>");
    Response.Write("格式(如是已带播放器的第三方地址，请此留空):<br/><input type=\"text\" name=\"book_ext\" value=\"\"/><br/>");
