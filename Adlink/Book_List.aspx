<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Book_List.aspx.cs" Inherits="KeLin.WebSite.adlink.Book_List" %><%@ Import namespace="KeLin.ClassManager.Tool" %><%
...
if (ver == "1")
{
    ...
    for (int i = 0; (listVo != null && i < listVo.Count); i++)
    {
        ...
        strhtml.Append(index + ".<a href=\"" + http_start + "adlink/book_view.aspx?siteid=" + siteid + "&amp;classid=" + listVo[i].book_classid + "&amp;id=" + listVo[i].id + "&amp;lpage="+CurrentPage+"" + "\">");


        strhtml.Append(WapTool.GetShowImg(listVo[i].book_title,"0","adlink"));
      
        
        
        strhtml.Append(book_click+"</a><br/>");
    }
    if (listVo == null)
    {
    ...
    if (action == "search")
    {
        strhtml.Append("<a href=\"" + this.http_start + "adlink/book_search.aspx?siteid=" + siteid + "&amp;classid=" + classid + "" + "\">返回搜索</a> ");
    }
...
else //2.0界面
{
    ...
    for (int i = 0; (listVo != null && i < listVo.Count); i++)
    {
        ...
        if (WapTool.ISAPI_Rewrite3_Open == "1")
        {
            strhtml.Append("<a href=\"" + http_start + "adlink-" + listVo[i].id+".html" + lpagetemp + "\">" + WapTool.GetShowImg(listVo[i].book_title, "0", "adlink") + book_click + "</a></div>");
       
        }
        else
        {
            strhtml.Append("<a href=\"" + http_start + "adlink/view.aspx?id=" + listVo[i].id + lpagetemp + "\">" + WapTool.GetShowImg(listVo[i].book_title, "0", "adlink") + book_click + "</a></div>");
        }
    }
