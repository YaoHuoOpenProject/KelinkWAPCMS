<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="admin_WAPadd3_add.aspx.cs" Inherits="KeLin.WebSite.novel.admin_WAPadd3_add" %><%@ Import namespace="KeLin.ClassManager.Tool" %><%
Response.Write(WapTool.showTop(this.dostate+this.GetLang("目录章节|目录章节|content"), wmlVo));//显示头                                                                                                                                                                       
if (ver == "1")
{
    Response.Write("<p>");    
    Response.Write(this.ERROR);
    Response.Write("<a href=\"" + this.http_start + "novel/admin_WAPadd3_add.aspx?action=class&amp;siteid=" + this.siteid + "&amp;classid=" + this.classid + "&amp;bookid=" + this.bookid + "&amp;id=" + this.id + "&amp;sid=" + this.sid1 + "-2-" + this.cs + "-" + this.lang + "-" + this.myua + "-" + this.width + "\">" + this.GetLang("进入WAP2.0界面管理|进入WAP2.0界面管理|wap2.0 admin") + "</a><br/>");
...
else //2.0界面
{
    ...
    if (this.INFO != "ADDOK" && this.INFO != "UPOK")
    {
        ...
        Response.Write("<form name=\"gt\" action=\"" + http_start + "novel/admin_WAPadd3_add.aspx\" method=\"post\">");
        ...
        Response.Write(this.GetLang("续传内容|续传内容|Info") + "*:<br/>");
